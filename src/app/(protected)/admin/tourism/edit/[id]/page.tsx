...
const formSchema = z.object({
  ...
  openingHours: z.object({
    weekday: z.string().min(5, "Weekday hours required"),
    weekend: z.string().min(5, "Weekend hours required"),
  }),
  landmarks: z.array(
    z.object({
      name: z.string().min(2, "Name must be at least 2 characters"),
...
export default function EditTourismPage({
  ...
  const { mutate: updateTourismItem } = useMutation({
    mutationFn: updateTourism,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["tourism"] });
      toast({
        title: "Success",
        description: "Tourism location updated successfully.",
      });
    ...
    onError: (error) => {
      ...
      toast({
        title: "Error",
        description: "Failed to update tourism location. Please try again.",
        variant: "destructive",
      });
    },
  });
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    try {
      if (!tourism) throw new Error("No tourism data available");

      const updatedTourism: TourismType = {
        id: tourism.id,
        name: values.name,
        type: values.type,
      ...
      updateTourismItem(updatedTourism);
    } catch (error) {
      console.error(error);
      toast({
        title: "Error",
  ...
  if (isLoading) return <TourismEditLoading />;
  if (isError)
    return (
      <ErrorState
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Edit Tourism Location</CardTitle>
        <CardDescription>Update the tourism location details</CardDescription>
      </CardHeader>
      <CardContent>
        <Form {...form}>
