...
import { getStudentResourceById } from "@/lib/api/student-resources";
import { StudentResource } from "@/types/student-resource";
import ViewResourceLoading from "./loading";
...
// ... existing mapStyles and SEOUL_BOUNDS constants ...
const mapStyles = [
  {
    featureType: "poi",
    elementType: "labels",
...
const mockReviews = [
  {
    ...
    date: "2024-02-15",
    comment:
      "Excellent facilities and supportive staff. Great learning environment!",
  },
  {
    id: 2,
    ...
    rating: 4,
    date: "2024-02-10",
    comment: "Good resources available, but can get crowded during peak hours.",
  },
];
...
const isCurrentlyOpen = (resource: StudentResource) => {
...
const getNextOpenTime = (resource: StudentResource) => {
...
const page = () => {
  ...
  useEffect(() => {
    if (!id) {
      redirect("/user/student-resources");
    }
  }, [id]);
  ...
  const { mutate: addToFavorite, isPending } = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: ["student-resources-favorited", resource?.id],
      });
      toast({
        title: "Success",
  ...
  const { data: isJobFavorited } = useQuery({
    queryKey: ["student-resources-favorited", resource?.id],
    queryFn: () => favoriteExists(resource?.id as string, userId as string),
    enabled: !!resource?.id,
  });
  ...
  const { mutate: removeFromFavorite, isPending: isRemoving } = useMutation({
    mutationFn:() => deleteFavoriteByItemId(resource?.id as string, userId as string),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["favorites"] });
      queryClient.invalidateQueries({
        queryKey: ["student-resources-favorited", resource?.id],
    ...
    onError: () => {
      toast({
        title: "Error",
        description: "Failed to remove resource from favorites",
        variant: "destructive",
      });
    },
  });
  ...
  const nextImage = () => {
    if (resource?.photos) {
      setCurrentImageIndex((prev) => (prev + 1) % resource.photos!.length);
    }
  };
  ...
  const prevImage = () => {
    if (resource?.photos) {
      setCurrentImageIndex(
        (prev) => (prev - 1 + resource.photos!.length) % resource.photos!.length
      );
    }
  };
  ...
  if (isLoading) return <ViewResourceLoading />;
  if (isError || !resource)
    return (
      <ErrorState
        title="Error loading resource"
        description="Failed to load student resource details"
        retryAction={() => refetch()}
      />
    );
              ...
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge
                      variant={
                        isCurrentlyOpen(resource) ? "default" : "secondary"
                      }
                      className={`${
                    ...
                    >
                      {isCurrentlyOpen(resource) ? "Open Now" : "Closed"}
                    </Badge>
                  </div>
                </div>
                {!isCurrentlyOpen(resource) && (
                  <p className="text-sm text-muted-foreground mt-2">
                    Opens tomorrow at {getNextOpenTime(resource)}
                  </p>
                )}
              </CardHeader>
              <CardContent>
